...
class GestorGenerosMusicales {
    ...
    public function recomendarCanciones($usuario_id, $limit = 10) {
        ...
        $query = "SELECT genero_nombre FROM favoritos_generos WHERE usuario_id = ?";
        $stmt = $this->pdo->prepare($query);
        $stmt->execute([$usuario_id]);
        $generos_favoritos = $stmt->fetchAll(PDO::FETCH_COLUMN, 0);
        
 if (empty($generos_favoritos)) {
            // Modificado para usar el campo genero en lugar de genero_id
...
$query = "SELECT c.* FROM canciones c 
                 WHERE c.genero IN (".str_repeat('?,', count($generos_favoritos)-1)."?) 
                 ORDER BY c.reproducciones DESC 
                 LIMIT ?";
...
class ColaRecomendaciones {
    ...
    private function cargarRecomendacionesPorGenero($gestorGeneros) {
        // Modificado para usar genero en lugar de genero_id
        ...
        if (empty($generos)) {
            // Cambiado de genero_id a genero_nombre
            $query = "SELECT genero_nombre as genero FROM favoritos_generos WHERE usuario_id = :usuario_id LIMIT 2";
            $stmt = $this->pdo->prepare($query);
            $stmt->bindParam(':usuario_id', $this->usuario_id);
            $stmt->execute();
            $generos = $stmt->fetchAll(PDO::FETCH_ASSOC);
