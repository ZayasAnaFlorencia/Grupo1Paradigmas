...
// ===================================================================
// EJEMPLO DE USO (sección opcional para pruebas)
// ===================================================================
...
if (isset($_SERVER['SCRIPT_FILENAME']) && basename(__FILE__) == basename($_SERVER['SCRIPT_FILENAME'])) {
    ...
    echo "<h1>Sistema de Generación de Playlists Automáticas</h1>";
    echo "<h2>Basado en Árbol de Géneros Musicales</h2>";
    ...
    try {
        ...
        if (empty($generosFavoritos)) {
            ...
            echo "<p>Géneros favoritos basados en historial:</p>";
        } else {
            echo "<p>Géneros favoritos explícitos:</p>";
        }
        
        echo "<ul>";
        foreach ($generosFavoritos as $genero) {
            $nombre = $genero['genero'] ?? $genero['nombre'];
            $jerarquia = $gestorGeneros->obtenerJerarquiaGenero($nombre);
            
            echo "<li><strong>{$nombre}</strong>";
            if (!empty($jerarquia)) {
                echo " (Jerarquía: " . implode(" → ", array_reverse($jerarquia)) . ")";
            }
            echo "</li>";
        }
        echo "</ul>";
        
        // Generar y mostrar playlist
        $nombrePlaylist = $generador->generarNombrePlaylist($generosFavoritos);
        $playlist = $generador->generarPlaylistAutomatica($usuarioId, $nombrePlaylist, 10);
        
        if ($playlist) {
            echo "<h2>Playlist generada: {$playlist['nombre']}</h2>";
            echo "<p>Número de canciones: {$playlist['num_canciones']}</p>";
            echo "<h3>Canciones:</h3>";
            echo "<ul>";
            foreach ($playlist['canciones'] as $cancion) {
                echo "<li>{$cancion['titulo']} - {$cancion['artista']} ({$cancion['genero']})</li>";
            }
            echo "</ul>";
        } else {
            echo "<p style='color: red;'>Error al generar la playlist automática</p>";
        }
    ...
    echo "<hr><small>Este es un ejemplo de prueba. Para integrar en tu sistema, usa las clases directamente.</small>";
}
?>